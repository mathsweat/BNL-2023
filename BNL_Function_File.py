...
def two_dim_remove_repeat(val_list_x,val_list_y): 
    result =[] 
    result2=[]
    for i in range(len(val_list_x)): #both value lists should have same length
        keep=0
        for j in range(len(result)): #both result lists should have same length
            if val_list_x[i]==result[j] and val_list_y[i]==result2[j]: #look at if the coordinate exists in our list already
                keep+=1 #if the coordinate exists we make keep 1
        if keep==0: #if coordinate doesn't exist already, put it in the lists
            result.append(val_list_x[i])
            result2.append(val_list_y[i])
